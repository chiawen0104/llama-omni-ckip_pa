...
for split in ["train", "dev", "test"]:
    ...
    with open(f"data/{split}.json", "r") as f1:
        s1_data = json.load(f1)

    with open(f"/work/twsmwhk715/speech_units/km_labels/{split}_0_1.km", "r") as f2:
    ...
    for i, item in enumerate(s1_data):
        ...
        transcript = item['conversations'][1]['value']
    ...
    with open(f"data/{split}_answers.json", "w", encoding="utf-8") as fa:
        for item in answers:
            fa.write(json.dumps(item, ensure_ascii=False) + "\n")

    print(f"Converted {len(answers)} entries")
